...
#ifdef SYNC
template<typename keyType>
bool batree<keyType>::searchSync(keyType k) {	//search k in root
	PNODE r = root;
	while (r) {
		int i = 0;
		if (r->getC(0))//for the leaf node
...
#ifdef SYNC
template<typename keyType>
void batree<keyType>::insertSync(QUERY& tmp) {	//insert the k into root
	proModify.lock();
	keyType k = tmp.getK();
	if (searchSync(k)) {
		proModify.unlock();
...
#ifdef SYNC
template<typename keyType>
void batree<keyType>::delSync(QUERY& tmp) {	//delete the k from root
	proModify.lock();
	keyType k = tmp.getK();
	if (searchSync(k)) {
		PNODE r = root;
