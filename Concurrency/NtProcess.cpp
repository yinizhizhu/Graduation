...
int main() {
	...
	BOOL bSuccess = CreateProcess(
		...
		FALSE, NULL, NULL, NULL, &startupInfo, &processInformation);
	if (bSuccess) {
		...
		if (hDll) {
			...
			if (QueryInformationProcess) {
				...
				cout << "Cannot get NtQueryInformationProcess funcion!\nError code:\t" << GetLastError() << endl;
			}
			FreeLibrary(hDll);
		}
		else {
			cout << "Cannot load ntdll.dll!\nError code:\t" << GetLastError() << endl;
		}
	}
	else
		cout << "Cannot start process.\n" << "Error code:\t" << GetLastError() << endl;
	return 0;
}