...
int APIENTRY _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance,
	...
	HANDLE hSemaphore = CreateSemaphore(NULL, int(PHILOSOPHER_COUNT / 2),
	...
	for (int iIndex = 0; iIndex < PHILOSOPHER_COUNT; iIndex++) {
		...
		if (CreateProcess(TEXT("..\\Debug\\Philosopher.exe"),
			szBuffer, NULL, NULL, FALSE, 0, NULL, NULL,
			&startupInfo[iIndex], &processInformation[iIndex]))
...
void FillEllipse(HWND hWnd, HDC hDC, int iLeft, int iTop, int iRight, int iBottom, int iPass) {
	HBRUSH hBrush = NULL;
	if (iPass)	hBrush = CreateSolidBrush(RGB(255, 0, 0));
